...
import {
  Eye, Edit3, MoreVertical, KeyRound, MapPin, Trash2, Power, FileStack, Route as RouteIcon, Save, X
} from "lucide-react";
...
/* ===== Menú en portal para que esté por encima de todo ===== */
...
function PortalMenu({
  ...
  state: PortalMenuState; onClose: () => void;
  ...
  const u = state.row;
  const body = (
    <div className="portal-menu" style={{ left: state.x, top: state.y }} onClick={(e)=>e.stopPropagation()}>
...
function EditUserModal({
    ...
    try {
      if (initial?.id) {
        const { error } = await supabase.from("users_local").update({
          username: form.username, nombre_completo: form.nombre_completo,
          rol: form.rol, estado: form.estado, correo: form.correo, telefono: form.telefono, ine: form.ine
  ...
  return (
    <div className="fixed inset-0 z-[10040] grid place-items-center bg-black/50" onClick={onClose}>
      <div className="modal-card modal-card-md" onClick={(e)=>e.stopPropagation()}>
        <div className="modal-head">
          <div className="text-[13px] font-medium">{initial?.id ? "Editar usuario" : "Crear usuario"}</div>
          <button className="btn-ghost !h-8 !px-3 text-xs" onClick={onClose}><X className="w-4 h-4" /> Cerrar</button>
        </div>
        <div className="p-4 grid sm:grid-cols-2 gap-3 text-[13px]">
          <label className="block">
            <div className="text-[12px] text-gray-600 mb-1">Usuario</div>
            <input className="input" value={form.username as string} onChange={(e)=>setForm(f=>({...f, username:e.target.value}))}/>
          </label>
          <label className="block">
            <div className="text-[12px] text-gray-600 mb-1">Nombre completo</div>
            <input className="input" value={form.nombre_completo as string} onChange={(e)=>setForm(f=>({...f, nombre_completo:e.target.value}))}/>
          </label>
          <label className="block">
            <div className="text-[12px] text-gray-600 mb-1">Rol</div>
            <select className="input" value={form.rol as string} onChange={(e)=>setForm(f=>({...f, rol:e.target.value as any}))}>
              <option>ADMIN</option>
...
function PasswordModal({ user, onSaved, onClose }: { user: UserRow; onSaved: () => void; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[10040] grid place-items-center bg-black/50" onClick={onClose}>
      <div className="modal-card modal-card-sm" onClick={(e)=>e.stopPropagation()}>
        ...
        <div className="px-4 py-3 border-t flex justify-end gap-2">
          <button className="btn-ghost !h-8 !px-3 text-xs" onClick={onClose}>Cancelar</button>
          <button className="btn-primary !h-8 !px-3 text-xs" onClick={submit} disabled={!pwd || pwd !== pwd2 || saving}>
            <Save className="w-4 h-4" /> Guardar
          </button>
        </div>
      </div>
    ...
    <div className="dt__card">
      ...
      <div className="dt__toolbar">
        <div className="dt__tools">
          ...
          <div className="flex justify-end">
            <button className="btn-primary btn--sm" onClick={(e)=>{ e.stopPropagation(); setCreateOpen(true); }}>
              + Crear nuevo
            </button>
          </div>
        </div>
      ...
      <div className="table-frame">
        <table className="min-w-full">
          ...
          <tbody>
            {rows.length === 0 ? (
              <tr><td colSpan={6} className="px-3 py-6 text-center text-[13px] text-gray-500">Sin resultados.</td></tr>
            ) : rows.map((u) => (
              <tr key={u.id}>
                ...
                <td className="text-[13px]">{u.estado === "ACTIVO" ? <span className="text-[var(--baci-blue)] font-medium">ACTIVO</span> : <span className="text-gray-500">INACTIVO</span>}</td>
                <td className="text-[13px]">{u.ine || "—"}</td>
                <td className="text-[13px]">{u.telefono || "—"}</td>
                <td>
                  <div className="flex justify-end gap-2">
                    <button className="btn-outline btn--sm" onClick={(e)=>{ e.stopPropagation(); setViewRow(u); }}>
                      <Eye className="w-3.5 h-3.5" /> Ver
                    </button>
                    <button className="btn-primary btn--sm" onClick={(e)=>{ e.stopPropagation(); setEditRow(u); }}>
      ...
      <PortalMenu
        ...
        onPassword={(u)=>setPassRow(u)}
        onAssignPobs={(u)=>setAssignPobsFor(u)}
        onAssignRoutes={(u)=>setAssignRoutesFor(u)}
        onToggle={async (u)=>{
        ...
        onDocs={(u)=>setDocsFor(u)}
        onDelete={async (u)=>{
          const ok = await confirm({
            title: "Eliminar usuario",
            message: <>Esta acción no se puede deshacer.<br/>¿Eliminar al usuario <strong>@{u.username}</strong>?</>,
      ...
      {docsFor && <UserDocumentsModal userId={docsFor.id} username={docsFor.username} onClose={()=>setDocsFor(null)} />}
      {assignPobsFor && <AssignPopulationsModal capturistaId={assignPobsFor.id} onClose={()=>setAssignPobsFor(null)} />}
      {assignRoutesFor && <AssignRoutesModal capturistaId={assignRoutesFor.id} onClose={()=>setAssignRoutesFor(null)} />}
