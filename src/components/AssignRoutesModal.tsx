...
export default function AssignRoutesModal({
  ...
  async function loadAssigned() {
    // join simple: capturista_rutas + rutas
  ...
  async function add(r: Ruta) {
    if (!confirm(`¿Asignar la ruta ${r.folio} — ${r.nombre}?`)) return;
    await assignRouteToCapturista(capturistaId, r.id);
    await loadAssigned();
  }

  async function remove(r: Ruta) {
    if (!confirm(`¿Quitar la ruta ${r.folio} — ${r.nombre}?`)) return;
  ...
  return (
    <div className="fixed inset-0 z-[9998] grid place-items-center bg-black/50" onClick={onClose}>
      <div className="w-[96vw] max-w-3xl bg-white rounded-2 border shadow-xl overflow-hidden" onClick={(e)=>e.stopPropagation()}>
                ...
                {searchRows.map(r => (
                  <li key={r.id} className="flex items-center justify-between px-3 py-2">
                    <div className="min-w-0">
                      <div className="text-[13px] font-medium truncate">{r.folio} — {r.nombre}</div>
                    </div>
                    <button
            ...
            {assigned.length === 0 ? (
              ...
              <>
                <ul className="divide-y">
                  {pageRows.map(r => (
                    <li key={r.id} className="flex items-center justify-between px-3 py-2">
                      <div className="text-[13px]">{r.folio} — {r.nombre}</div>
