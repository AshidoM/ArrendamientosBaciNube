...
function DataTable<RowT>({
  ...
  useEffect(() => {
    setPage(1);
  }, [query, effectivePageSize, rows.length]);
  ...
  return (
    <div className="grid gap-3">
      {/* Toolbar de búsqueda (compacta) */}
      {enableSearch && (
        <div className="relative w-full sm:max-w-xs mt-2">
          <input
            className="input dt__search--sm dt_search--sm"
          ...
          <ul className="divide-y">
            {pageRows.map((row, idx) => {
              ...
              return (
                <li
                  key={rid || idx}
                  className="grid grid-cols-12 items-center px-3 py-2 relative"
                >
                  {columns.map((c) => (
                    <div
                      key={c.key}
                  ...
                  <div className="col-span-6 sm:col-span-1 flex justify-end gap-1 relative">
                    {primaryAction && (
                      <button
                        className={
                          primaryAction.buttonClassName ??
                          "btn-primary !h-8 !px-2 text-xs"
                        }
                    ...
                    {menuActions.length > 0 && (
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          openMenuFor.current =
                            openMenuFor.current === rid ? null : rid;
                          force((v) => v + 1);
                        }}
                      >
          ...
          <div className="flex items-center gap-2">
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault();
                const v = Number(
                  (e.currentTarget.elements.namedItem("pagenum") as HTMLInputElement)
                    .value
                );
